...
FVM_changestate(VAR(PduInfoType,COMSTACK_TYPES_VAR) RxPduId){
	...
	*/
	if(RxPduId == ackvid ){
		verifystate=1;
	}else if(RxPduId == notifyid ){
		secocenabled = 1;
	}else if(RxPduId == errorid ){
...
FVM_updateTrip(P2CONST(PduInfoType,SLAVE_CODE,SLAVE_APPL_CONST) PduInfoPtr){
	/*
		...
		2.解析收到的trip报文，重新构造验证mac所需的原始数据dataptr
			PduInfoPtr.data = trip(TripCntLengthgth)+1(1bit)+ mac(64bit-TripCntLengthgth-1);
			...
			例如  TripCntLengthgth=12  [ 0x02 0xbd 0x00 0x14]  
...
/*
	...
	2.通过比较各计数器上一次发送值和当前值，构造新鲜值。 参考图
